...
private:
    ...
    void checkVulkanValidationLayerSupport();
    ...
    void findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties, uint32_t* output);
    void copyVulkanBuffer(VkBuffer src, VkBuffer dst, VkDeviceSize size);

    // Depth buffer helper functions
    void findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features, VkFormat& format);
...
public:
    ...
    template <typename T>
    void bindVertices(std::vector<T> vertices, VkBuffer& vertexBuffer, VkDeviceMemory& vertexBufferMemory);
    ...
    void createImageView(VkImage image, VkImageViewType viewType, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t numLayers, VkImageView& imageView);
    void createVulkanImage(uint32_t width, uint32_t height, uint32_t layers,
...
template <typename T>
void vulkan::bindVertices(std::vector<T> vertices, VkBuffer& vertexBuffer, VkDeviceMemory& vertexBufferMemory) {
